...
export class StackDetailsComponent implements OnChanges {
    ...
    private ifManifestHasWarning(manifest: ResultInformationModel): boolean {
        let isSecurityWarning =
            this.reportSummaryUtils.getSecurityReportCard(manifest.user_stack_info).hasWarning;
        let isInsightsWarning =
            this.reportSummaryUtils.getInsightsReportCard(manifest.recommendation).hasWarning;
        let isLicenseWarning =
            this.reportSummaryUtils.getLicensesReportCard(manifest.user_stack_info).hasWarning;
        console.log('does manifest has warning:',
            isSecurityWarning || isInsightsWarning || isLicenseWarning ? true : false);
